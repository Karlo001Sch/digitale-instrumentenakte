...
interface ToastContextValue {
  toast: (props: Omit<ToastData, 'id' | 'open'>) => void
}
...
export function ToastProvider({ children }: { children: ReactNode }) {
  ...
  const toast = useCallback((props: Omit<ToastData, 'id' | 'open'>) => {
      ...
      <ToastPrimitive.Provider>
        ...
        {toasts.map((t) => (
          <ToastPrimitive.Root
            ...
            className={cn(
              'fixed bottom-4 right-4 z-[100] flex items-start gap-3 rounded-lg border p-4 shadow-lg bg-background w-80',
          ...
          >
            ...
            <ToastPrimitive.Close className="text-muted-foreground hover:text-foreground flex-shrink-0">
              <X className="h-4 w-4" />
